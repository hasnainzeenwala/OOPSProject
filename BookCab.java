...
public class BookCab extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==bookCab) {
			...
			if((pickUpIndex != dropOffIndex) && !(walletBalanceAmount < 300) && !(walletBalanceAmount - tripFare < 0)) {
				...
				try {
				    ...
				    {   driver=rs.getString("Name");
				    	...
				    	success=1;
				    	CabBooked cabBookedFrame = new CabBooked(user, driver, rating, tripFare);
			...
			else if((walletBalanceAmount < 300) || (walletBalanceAmount - tripFare < 0)) {
				int min=tripFare-walletBalanceAmount;
				JOptionPane.showMessageDialog(this, "Insufficient Funds.You need to add minimum funds of " + min);
			}
		}
