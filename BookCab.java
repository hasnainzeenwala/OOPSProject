...
public class BookCab extends JFrame implements ActionListener{
	...
	private JLabel dropOff;
    	...
    	int fareArray[] = new int[100];
    	
	final int noLocations = 6;
    	private String locations[] = {"Masoodpur", "Khatiwala Tank", "Sapna Sangeeta Road", "Manik Bagh Road", "Saket Nagar", "Thumkunta"};
	...
	public BookCab(String userName) {
        ...
        balance.setSize(350, 20);
        balance.setLocation(50, 500);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==bookCab) {
			...
			if((pickUpIndex != dropOffIndex) && !(walletBalanceAmount < 300) && !(walletBalanceAmount - tripFare < 0)) {
				this.setVisible(false);
				//create variables for driver name 
				try {
					Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/cab", "root", "password");
				    Statement stmt=con.createStatement();
				    ResultSet rs=stmt.executeQuery("SELECT * FROM driverdetails WHERE Area="+pickUpIndex+" ORDER BY Rating  DESC;");
				    while(rs.next() && success==0)
				    {   driver=rs.getString("Name");
				        rating=rs.getDouble("Rating");
				        mobile=rs.getString("Mobile");
				    	success=1;
				    	CabBooked cabBookedFrame = new CabBooked(user, driver, rating, tripFare,mobile);
				 ...
				 try {
	    				Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/cab", "root", "password");
	    			    PreparedStatement stmt=(PreparedStatement) con.prepareStatement("UPDATE driverdetails SET Area=? WHERE Name=?");
	    			    stmt.setInt(1,dropOffIndex);
			...
			else if(walletBalanceAmount < 300) {
				int min=300-walletBalanceAmount;
				JOptionPane.showMessageDialog(this, "Insufficient Funds.You need to add minimum funds of " + min + " to look for a cab");
			}
			else if(walletBalanceAmount - tripFare < 0) {
				int min=tripFare-walletBalanceAmount;
				JOptionPane.showMessageDialog(this, "Insufficient Funds.You need to add minimum funds of " + min + " to pay for this trip");
			}
		}
